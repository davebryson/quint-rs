...
fn setup_evaluator() {
    ...
    .unwrap();
    let run_def = parsed.find_definition_by_name("expr").unwrap();
    let mut interpreter = Interpreter::new(&parsed.table);
    let mut env = Env::new(Rc::clone(&interpreter.var_storage));

    //
    let expected = interpreter.eval(&mut env, run_def.expr.clone()).unwrap();
    let value = expected.normalize();
    println!("value: {:?}", value);
